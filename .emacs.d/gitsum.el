...
(defun gitsum-add ()
  "Add the patch as-is to working tree."
  (interactive)
  (when (y-or-n-p (concat "Are you sure you want to add changes? "))
    (when (eq (shell-command-on-region (point-min) (point-max) "git apply --cached --unidiff-zero") 0)
      (gitsum-refresh))))

(defun gitsum-commit ()
  "Commit the patch as-is, asking for a commit message."
  (interactive)
  (shell-command-on-region (point-min) (point-max) "git apply --check --cached --unidiff-zero")
...
(defun gitsum-revert ()
  ...
  (let ((count (count-matches "^@@" (point-min) (point-max))))
    (if (not (yes-or-no-p
      ...
      (when (eq (shell-command-on-region (point-min) (point-max) "git apply --reverse --unidiff-zero") 0)
        (gitsum-refresh)))))

(defun gitsum-do-commit ()
  "Perform the actual commit using the current buffer as log message."
  (interactive)
  (when (eq (with-current-buffer log-edit-parent-buffer
...
(defun gitsum-switch-from-git-status ()
  "Switch to gitsum, restricting diff to marked files if any."
  (interactive)
